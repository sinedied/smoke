...
function processRequest(options) {
  return async (req, res, next) => {
    ...
    const matches = mocks.reduce((allMatches, mock) => {
      ...
      if (match) {
        const isJsMock = mock.ext === 'js' || mock.ext === 'cjs';
        const accept = req.accepts(mock.type);

        // For JS mocks, skip accept header validation (they can handle it themselves)
        ...
        if ((isJsMock || accept) && matchMock(mock, method, options.set, query)) {
          const score =
            ...
            (accept ? 0.1 : 0); // Prefer non-JS mocks that match accept header
    ...
    if (matches.length === 0) {
      ...
      // Find the best match, preferring non-JS mocks that match accept header
      let selectedMatch = sortedMatches[0]; // Default to highest scored match

      if (accept) {
        const acceptMatch = sortedMatches.find((match) => accept === match.mock.type);
        if (acceptMatch) {
          selectedMatch = acceptMatch;
        }
      }

      const {match, mock} = selectedMatch;
