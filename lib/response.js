...
async function respondMock(res, mock, data, statusCode = null) {
  let result;
  const hasData = mock.data !== undefined;
  ...
  if (mock.isTemplate || mock.ext === 'json') {
    try {
      result = hasData ? mock.data : await fs.readFile(mock.file, 'utf-8');
    } catch (error) {
    ...
    try {
      ...
      result = hasData ? mock.data(data) : importFresh(filePath)(data);
    } catch (error) {
    ...
    try {
      // Read file as buffer
      result = hasData ? mock.data : await fs.readFile(mock.file);
    } catch (error) {
      return internalError(res, `Error while reading mock file "${mock.file}"`, error);
    }
  }
